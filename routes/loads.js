...
router.get('/my-loads', ensureAuthenticated, async (req, res) => {
  try {
    await closeExpiredLoads();
    const user = await User.findById(req.user._id).populate('loadsPosted');
...
router.get('/:id', async (req, res) => {
  try {
    const load = await Load.findById(req.params.id).populate('postedBy', 'username firstName lastName').populate('bids', 'amount bidder message status').populate('bids.bidder', 'username');
